...
public class ReflectUtils {
	...
	public static void setFieldValue(Object model, String prepertyName, String value) {
		...
		try {
			model.getClass()
					.getMethod("set" + prepertyName.substring(0, 1).toUpperCase() + prepertyName.substring(1), plusPara)
					.invoke(model, transPlusPara);
		} catch (Exception e) {
			throw ReflectUtils.convertReflectionExceptionToUnchecked(e);
		}
	}
	...
	public static void setFieldValue(Object model, Class<?> propertyClass, String propertyName, Object propertyValue) {
		...
		try {
			model.getClass().getMethod("set" + StringUtils.capitalize(propertyName), plusPara).invoke(model,
					transPlusPara);
		} catch (Exception e) {
			throw ReflectUtils.convertReflectionExceptionToUnchecked(e);
		}
